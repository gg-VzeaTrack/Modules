...
local function AttackAll()
    ...
    if #enemies > 0 then
        ...
        for i, v in pairs(enemies) do
            if not args[1] then args[1] = v.Head
            end
...
function FastAttack:IsEntityAlive(entity)
    ...
    if ok and typeof(health) == "number" then
        return health > 0
    end
    local valueHealth = humanoid:FindFirstChild("Health")
    if valueHealth and valueHealth:IsA("NumberValue") then
        return valueHealth.Value > 0
    end
    return false
end
        ...
        if ok and info and info.name then
            ...
            if name == "Attack" or name == "attack" or name == "RegisterHit" then
                ...
                old = hookfunction(v, function(...)
                    if AttackInstance and AttackInstance.Attack then
                        AttackInstance:Attack()
                    end
                    return old(...)
                end)
