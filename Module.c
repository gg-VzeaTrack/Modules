...
Module.is_next_Doors = function()
    if game.PlaceId ~= 73902483975735 then return end
    local lastDungeonEntrance = nil
    for i = 1, 15 do
        local dungeon = workspace.Map.Dungeon:FindFirstChild(tostring(i))
        if dungeon then
            local entrance = dungeon:FindFirstChild("EntranceTeleporter")
            if entrance and entrance.Root then
                lastDungeonEntrance = entrance.Root
            end
        end
    end
    return lastDungeonEntrance
end
Module.LowCPU = function()
