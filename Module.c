...
function Module.BringEnemy()
    ...
    for _, Enemy in pairs(workspace.Enemies:GetChildren()) do
        ...
        if Enemy.Name ~= PosName then continue end
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if method == "InvokeServer" and tostring(self) == "CommF_" then
        local ret = old(self, ...)

        -- getInventory
        if args[1] == "getInventory" and type(ret) == "table" then
            for _, v in pairs(ret) do
                if v.Type == "Material" and v.Name == "Bones" then
                    Cache.Bones = v.Count or 0
                end
            end
        end

        -- Elite Hunter
        if args[1] == "EliteHunter" and args[2] == "Progress" then
            Cache.Elite = tonumber(string.match(ret or "", "%d+")) or Cache.Elite
        end

        -- Spy
        if args[1] == "InfoLeviathan" then
            Cache.Spy = tonumber(string.match(ret or "", "%d+")) or Cache.Spy
        end

        -- Cake Prince
        if args[1] == "CakePrinceSpawner" then
            Cache.Cake = tonumber(string.match(ret or "", "%d+")) or Cache.Cake
        end
...
function Module.Process(Value)
    if Value == "Bones" then
        return Cache.Bones
    elseif Value == "EliteHunter" then
        return Cache.Elite
    elseif Value == "SpyChecker" then
        return Cache.Spy
    elseif Value == "Katakuri" then
        return Cache.Cake
    end
    return 0
end
